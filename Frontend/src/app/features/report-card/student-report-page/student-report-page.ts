...
@Component({
  ...
  imports: [RouterLink, MatIcon, StudentAvatar, Dropdown, CommentCard],
...
export class StudentReportPage implements OnInit {
  ...
  protected readonly comment = computed(() => {
    ...
    return rc?.comment?.trim() ? rc.comment! : null;
  });
  ...
  protected saveComment(text: string): void {
    ...
    this._reportCardService
      .saveComment(this._studentId, pid, { comment: text })
  ...
  ngOnInit(): void {
    ...
    this._evaluationService
      .getAll()
      .subscribe((list) => this.evaluations.set(list.filter((e) => e.student.id === this._studentId)));
